...
function bridgeChat(bot, player, msg, event) {
	if (
		...
		Settings.botName.toLowerCase() !==
			ChatLib.removeFormatting(bot).toLowerCase()
	)
		return;
	cancel(event);
	...
	let combined = addLinks(
		`ยง2Guild > ${bridgeColor + bridgeBold + bridgeText} ${
			discordColor + discordBold + player
		}ยงr: ${msg}`,
		event
	);
...
register('chat', (...args) => {
	...
	let player = args[1];
	cancel(event);
	let formattedMessage = ChatLib.addColor(
		`&2Guild > &a[VIP] MAChatbridge &6[EQUITE] &f${player} » ${decryptedLink}`
	);
	ChatLib.chat(formattedMessage);
}).setCriteria(
	'&r&2Guild > ${*} ${bot} ${*}: &r${player}' +
		Settings.seperator +
		' [Image] ${msg}&r'
);
